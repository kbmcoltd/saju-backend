...
function buildFortunePrompt({ element, dayStem, dayBranch, yearStem, yearBranch, monthStem, monthBranch, hourStem, hourBranch }) {
...
app.post('/api/fortune', async (req, res) => {
  try {
    ...
    const promptText = buildFortunePrompt(req.body);
    ...
    if (items.length === 0) {
      return res.status(502).json({ error: 'AI로부터 유효한 운세 결과를 받지 못했습니다.' });
    }
